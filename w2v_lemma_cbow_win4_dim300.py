...
vector_size = 300
sg = 0  # CBOW
output_csv = f"similarity_lemma_cbow_win{window_size}_dim{vector_size}.csv"
model_file = f"model_lemma_cbow_win{window_size}_dim{vector_size}.model"
...
# === Eğitim Verisini Genişlet: Şikayet + Teşhis
sentences = df['complaint_lemma'].tolist() + df['diagnosis_lemma'].tolist()
...
# === Vektörleri Hesapla
df['complaint_vec'] = df['complaint_lemma'].apply(get_mean_vector)
df['diagnosis_vec'] = df['diagnosis_lemma'].apply(get_mean_vector)
